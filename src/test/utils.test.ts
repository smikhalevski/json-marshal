...
import { manualDecodeBase64, manualEncodeBase64 } from '../main/utils.js';
...
test('manualEncodeBase64', () => {
  expect(manualEncodeBase64(new TextEncoder().encode('Hello world').buffer)).toBe('SGVsbG8gd29ybGQ=');
  expect(manualEncodeBase64(new TextEncoder().encode('Man').buffer)).toBe('TWFu');
  expect(manualEncodeBase64(new TextEncoder().encode('Ma').buffer)).toBe('TWE=');
  expect(manualEncodeBase64(new TextEncoder().encode('Hello worlds!').buffer)).toBe('SGVsbG8gd29ybGRzIQ==');
  expect(manualEncodeBase64(createRangeBuffer())).toBe(
    ''
  );
});

test('manualDecodeBase64', () => {
